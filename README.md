...
### Classic References
- **POSIX Threads (IEEE Std 1003.1c)**: Thread lifecycle, mutexes (`pthread_mutex_t`), and condition variables (`pthread_cond_t`, `pthread_cond_timedwait`).
...
- Analyzing the project specification (`codexion.pdf`) and grading evaluation scale (`Intra Projects Codexion Edit.pdf`).
...
The subject mandates that burnout logs be emitted within **10 ms** of the actual occurrence. A dedicated `monitor` thread samples coders at sub-millisecond intervals (500 µs), immediately flagging missed deadlines, broadcasting wakeup signals to dormant threads, and terminating the simulation safely.
...
2. **`pthread_mutex_t print_mutex`**: Enforces sequential terminal printing to prevent log tearing.
...
Each dongle maintains its own `t_heap` array representing a min-heap. When a coder desires a dongle, `heap_push` queues the request according to the active policy:
...
When a coder finishes compiling, `release_dongles` sets `available_at = get_time_ms() + cooldown` and invokes `pthread_cond_broadcast` while holding `dongle.mutex`. Waiting threads woken up re-evaluate their priority in the heap and verify that the cooldown has elapsed before acquiring the dongle, eliminating any race between concurrent requesters.
