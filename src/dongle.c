...
static int take_dongle(t_coder *coder, int dongle_id)
{
    ...
    while(is_stopped(data))
    {
        ...
        if(!(d->taken) && (top_request(&d->heap) == coder.id))
        {
            ...
            if (get_time_ms() > d.available_at)
            {
                ...
                pthread_metux_unlock(&d->mutex)
                print_status(coder, "has taken a dongle");
                return (1);
            }
            //if still not avaible 
            ts.tv_sec = d.available_at / 1000;
            ts.tv_usec = (d.available_at % 1000) * 1000000;
